                    ...
                    if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                        ...
                        if command -v ~/.acme.sh/acme.sh &> /dev/null && ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
                            ~/.acme.sh/acme.sh --installcert --force -d "${domain}" \
                                ...
                                --reloadcmd "prx-panel restart" 2>&1 || true
...
ssl_cert_issue() {
    ...
    reloadCmd="prx-panel restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}prx-panel restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; prx-panel restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; prx-panel restart"
                reloadCmd="systemctl reload nginx ; prx-panel restart"
                ;;
            2)
                LOGD "It's recommended to put prx-panel restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; prx-panel restart): " reloadCmd
...
# needs this:   prx-panel pgclient <major>
pg_upgrade_client() {
    local want="$1" have
...
migrate_db() {
    ...
    if [[ ! -f "$input" ]]; then
        ...
        echo -e "Usage: ${green}prx-panel migrateDB [file.db|file.dump] [output]${plain}"
        return 1
    fi
...
|  ${blue}prx-panel restart-xray${plain}          - Restart Xray                     │
│  ${blue}prx-panel status${plain}                - Current Status                   │
│  ${blue}prx-panel settings${plain}              - Current Settings                 │
│  ${blue}prx-panel enable${plain}                - Enable Autostart on OS Startup   │
│  ${blue}prx-panel disable${plain}               - Disable Autostart on OS Startup  │
│  ${blue}prx-panel log${plain}                   - Check logs                       │
│  ${blue}prx-panel banlog${plain}                - Check Fail2ban ban logs          │
│  ${blue}prx-panel update${plain}                - Update                           │
│  ${blue}prx-panel update-dev${plain}            - Update to Dev channel (latest)   │
│  ${blue}prx-panel update-all-geofiles${plain}   - Update all geo files             │
│  ${blue}prx-panel migrateDB [file]${plain}      - Convert .db <-> .dump (SQLite)   │
│  ${blue}prx-panel pgclient [ver]${plain}        - Upgrade pg_dump/pg_restore tools │
│  ${blue}prx-panel legacy${plain}                - Legacy version                   │
│  ${blue}prx-panel install${plain}               - Install                          │
│  ${blue}prx-panel uninstall${plain}             - Uninstall                        │
└────────────────────────────────────────────────────────────────┘"
}
